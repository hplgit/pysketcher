...
def given_inferred(func):
    ...
    if varargs:
        return invalid(
            "Cannot apply @given_inferred to a function with arbitrary positional arguments."
        )
    if varkw:
        return invalid(
            "Cannot apply @given_inferred to a function with arbitrary keyword arguments"
        )
