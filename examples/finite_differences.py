...
axis = Composition({
        ...
        set_line_color(Style.Color.BLACK).set_line_width(1),
...
forward_tangent = Line(p, pf).interval(x_range=domain2). \
    set_line_style(Style.LineStyle.DASHED).set_line_color(Style.Color.RED)
backward_tangent = Line(pb, p).interval(x_range=domain). \
    set_line_style(Style.LineStyle.DASHED).set_line_color(Style.Color.GREEN)
centered_tangent = Line(pb, pf).interval(x_range=domain). \
    set_line_style(Style.LineStyle.DASHED).set_line_color(Style.Color.BLUE)
...
exact_tangent = Line(Point(x + h, f(x + h)), Point(x - h, f(x - h))). \
    ...
    set_line_style(Style.LineStyle.DOTTED).set_line_color(Style.Color.BLACK)
