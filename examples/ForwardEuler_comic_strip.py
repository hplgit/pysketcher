...
def make_fig(dt=0.5, heading=""):
    ...
    circles[0] = (
        Circle((t_next, u_next), 0.05).set_linecolor("red").set_filled_curves("red")
    )
    ...
    text_exact = ArrowWithText(
        "exact solution", (2.5, 1), (2.5, I * np.exp(-a * 2.5)), alignment="left"
    )

    text_predict = ArrowWithText(
        "Here we know the slope:\n$u'=f(u,t)$!\nLet the solution continue\nalong that slope.",
        (1.7, 1.7),
        (t[-1], u[-1]),
        alignment="left",
    )
    text_next = ArrowWithText(
        "This is the next\npredicted point",
        (1, 0.25),
        (t_next, u_next),
        alignment="left",
...
fig = make_fig(
    dt=0.5,
    heading="Differential equations $u'=f(u,t)$\nare hard to solve,\nbut not with programming!",
)
fig.draw()
...
drawing_tool.erase()
fig = make_fig(
    dt=0.24, heading="Just reduce the time step\nto make more accurate\npredictions!"
)
fig.draw()
drawing_tool.display()
